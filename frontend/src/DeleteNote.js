...
const DeleteNote = () => {
  ...
  const handleDelete = async(e) => {
    e.preventDefault();
    try {
      await axios.delete(`http://127.0.0.1:8000/api/notes/delete/${note.id}`, { headers: { Authorization: `Bearer ${token}` }, data: note });
      navigate("/notes");
    } catch (error) {
  ...
  return (
    <div className="flex flex-col min-h-0">
      <main className="flex-grow">
        <form onClick={handleDelete} aria-label="deleteNote">
